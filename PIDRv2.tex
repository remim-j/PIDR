...
Coefficient devant l'opinion initiale & $I-A$ & $0$ & L'opinion initiale n'est pas prise en compte dans l'article \cite{VMG} \tabularnewline
		\hline
Coefficient devant la réponse de l'individu à l'instant précédent & $A(A-I)$ & $(I-A'(t-1))$ & L'influence d'un individu sur un autre ne dépend pas de l'individu \tabularnewline
		\hline
...
Afin de tester la validité du modèle de Friedkin et Johnson \cite{FJ}, nous allons utiliser les données obtenues grâce à l’expérience menée par Van de Kerckhove et al. \cite{VMG}. Nous utiliserons dans un premier temps les résultats obtenus lors du jeu de comptage, puis ceux obtenus dans le jeu d’évaluation des couleurs.\\

On rappelle que selon le modèle de Friedkin et johnson, nous avons \cite{FJ} :\\
...
En considérant uniquement cette formule au rang $t = 2$, nous obtenons :\\
\begin{equation}
\label{1}
y(2) =AWy(1)+(I-A)y(1), 
\end{equation}

qui peut aussi s’écrire de la forme :\\
\begin{equation}
\label{2}
mettreFormuleRegLineaireIci,
\end{equation}

Grâce à l’expérience menée par Van de Kerckhove et al. \cite{VMG}, nous connaissons $y(0)$, $y(1)$ et $y(2)$. Ces trois valeurs sont les réponses données par les sujets durant les trois tours du jeu. Nous n’avons pas ajouté de filtre supplémentaire aux données que nous utilisons. Nous pouvons alors faire une régression linéaire multiple afin de déterminer les valeurs de $\alpha$ et de $\beta$. Nous cherchons à savoir si le paramètre [?] est nul.\\
